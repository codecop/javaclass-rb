#! ruby
require 'test/unit'
require File.expand_path(File.dirname(__FILE__) + '/jar_class_list')

class TestJarClassList < Test::Unit::TestCase
  
  JAR = File.expand_path("#{File.dirname(__FILE__)}/../../test/demo.jar")
  ...
  def setup
    @cs = JarClassSearcher.new
  end
  
  def test_find_jars
    jars = @cs.find_jars('test')
  ...
  def test_overall
    ...
    list = @cs.compile_list(2, 'test', MockList.new )
